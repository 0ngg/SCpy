...
class user:
    def __init__(self, *args):
        ...
        dirname = os.getcwd()
        self.inits = pd.read_csv(dirname + "\\data\\test\\" + args[0])
        self.solid_props = pd.read_csv(dirname + "\\data\\test\\" + args[1])
        self.constants = pd.read_csv(dirname + "\\data\\test\\" + args[2])
...
class props:
    def __init__(self, *args):
        ...
        for i in args[0]:
            # fluid
            if "rho" == i:
                rho__ = 1 / HAPropsSI("Vha", "P", args[1], "T", args[2], "W", args[3])
                self.rho = np.array([rho__, rho__], dtype = float)
            elif "miu" == i:
                miu__ = HAPropsSI("mu", "P", args[1], "T", args[2], "W", args[3])
                self.miu = np.array([miu__, miu__], dtype = float)
            elif "cp" == i:
                cp__ = HAPropsSI("cp_ha", "P", args[1], "T", args[2], "W", args[3])
                self.cp = np.array([cp__, cp__], dtype = float)
            elif "alpha" == i:
                k__ = HAPropsSI("k", "P", args[1], "T", args[2], "W", args[3])
                rho__ = 1 / HAPropsSI("Vha", "P", args[1], "T", args[2], "W", args[3])
                cp__ = HAPropsSI("cp_ha", "P", args[1], "T", args[2], "W", args[3])
    ...
    def __appendBound(self, *args):
        ...
        if "fluid" == args[2]:
            for i in ["P", "Pcor", "u", "v", "w", "k", "e", "T"]:
                self.value.add(i, np.array([args[1].inits.loc[0, i], args[1].inits.loc[0, i]], \
                                 dtype = float))
                self.grad.add(i, np.array([np.array([0.00, 0.00, 0.00], dtype = float), \
                                np.array([0.00, 0.00, 0.00], dtype = float)]))
            self.prop = props(np.array(["rho", "miu", "cp", "alpha"]), \
                        args[1].inits.loc[0, "P"], args[1].inits.loc[0, "T"], \
                        args[1].constants.loc[0, "Wamb"])
        elif "solid" == args[2]:
            self.value.add(i, np.array([args[2].inits.loc[0, "T"], args[2].inits.loc[0, "T"]], \
                             dtype = float))
            self.grad.add("T", np.array([np.array([0.00, 0.00, 0.00], dtype = float), \
                            np.array([0.00, 0.00, 0.00], dtype = float)]))
            ...
            for i in ["P", "Pcor", "u", "v", "w", "k", "e", "T"]:
                ...
                self.grad.add(i, np.array([np.array([0.00, 0.00, 0.00], dtype = float), \
                                np.array([0.00, 0.00, 0.00], dtype = float)]))
            constant_solid_prop = dict({"k": args[1].solid_prop.loc["k", args[3]], "eps": args[1].solid_prop.loc["eps", args[3]]})
            self.prop = props(np.array(["rho", "miu", "cp", "alpha", "k", "eps"]), \
                        args[1].inits.loc[0, "P"], args[1].inits.loc[0, "T"], \
    ...
    def __appendDomain(self, *args):
        ...
        if "fluid" in args[0]:
            for i in ["P", "Pcor", "u", "v", "w", "k", "e", "T"]:
                ...
                self.prop = props(np.array(["rho", "miu", "cp", "alpha"]), \
                              args[1].inits.loc[0, "P"], args[1].inits.loc[0, "T"], \
                              args[1].inits.loc[0, "W"])
        else:
            self.value.add(i, np.array([args[1].inits.loc[0, "T"], args[1].inits.loc[0, "T"]], \
                             dtype = float))
            self.grad.add("T", np.array([np.array([0.00, 0.00, 0.00], dtype = float), \
                            np.array([0.00, 0.00, 0.00], dtype = float)]))
    ...
    def __call__(self, *args):
        ...
        if args[3] is True:
            return np.sqrt(np.sum(np.array( \
                          [map(lambda x: x^2, \
                          self.__dict__[args[0]][args[1]][args[2]])]))) 
        else:
...
class mesh:
    def __init__(self, *args):
        ...
        dirname = os.getcwd()
        spec = meshio.read(dirname + "\\data\\test\\" + args[0])
        self.getElements(spec)
        self.getInfo(spec, args[1])
    def __getMember(self, *args):
        # args nodes : np.array([], dtype = int)
        member__ = []
        for i in self.faces.keys():
            if all(np.array(list(map(lambda x: x in self.faces[i].node, args[1])))) is True:
                member__.extend([i])
        return np.array(member__)
    def __getSize(self, *args):
        # args : isface : bool, nodes / faces : np.array([], dtype = int)
        # triangle and quad area-based only for now
        size__ = 0.00
        centroid__ = np.array([0.00, 0.00, 0.00], dtype = float)
        if args[0] is True:
            # face
            if args[1].shape[0] == 3:
                # triangle
    ...
    def __getElements(self, *args):
        # args : mesh : meshio.mesh
        self.nodes = dict({})
        for i in range(0, mesh.points.shape[0]):
            self.nodes.add(i, mesh.points[i])
        self.faces = dict({}); face_id__ = 0; 
        for i in mesh.cells_dict.keys():
            if mesh.cells_dict[i][0].shape[0] < 5:
                # face
                for j in mesh.cells_dict[i]:
                    fnode__ = j
                    farea__, fcentroid__ = self.getSize(True, j)
                    face__ = face()
                    face__.appendElement(fnode__, fcentroid__, farea__)
                    self.faces.add(j + face_id__, face__)
                face_id__ += mesh.cells_dict[i].shape[0]
            else:
                continue
        ...
        for i in mesh.cells_dict.keys():
            if mesh.cells_dict[i][0].shape[0] >= 5:            
                ...
                for j in mesh.cells_dict[i]:
                    ...
                    cell__ = cell()
                    cell__.appendElement(cnode__, cface__, ccentroid__, cvolume__)
                    self.cells.add(j + cell_id__, cell__)
                cell_id__ += mesh.cells_dict[i].shape[0]
            else:
                continue
        return
    def __getTemplate(self):
        isbound = dict({})
        for i in range(0, self.cells.keys()):
            isbound.add(i, self.cells[i].member)
        row_neigh__ = dict({}); col_neigh__ = dict({}); data_neigh__ = dict({})
        row_bound__ = dict({}); col_bound__ = dict({}); data_bound__ = dict({})
        for i in range(0, self.cells.keys()-1):
            domain1__ = list(["fluid", "solid"])[[any(list(map(lambda x: x in k, \
                        self.cells[i].domain))) for k in ["fluid", "solid"]]]
            for j in range(i, self.cells.keys()):
                domain2__ = list(["fluid", "solid"])[[any(list(map(lambda x: x in k, \
                            self.cells[j].domain))) for k in ["fluid", "solid"]]]
                check = list(map(lambda x: x in self.cells[j].member, \
                        self.cells[i].member))
                    ...
                    if len(np.array([domain1__, domain2__])) != \
                       len(np.unique(np.array([domain1__, domain2__]))):
                        if domain1__ not in list(row_neigh__.keys()):
                            row_neigh__.add(domain1__, [i]); row_neigh__[domain1__].append(j)
                            col_neigh__.add(domain1__, [j]); col_neigh__[domain1__].append(i)
                            data_neigh__.add(domain1__, [neighbor__]); data_neigh__[domain1__].append(neighbor__)
                            row_bound__.add(domain1__, []); col_bound__.add(domain1__, []); data_bound__.add(domain1__, [])
                        else:
                        ...
                        if "conj" not in list(row_neigh__.keys()):
                            row_neigh__.add("conj", [i]); row_neigh__["conj"].append(j)
                            col_neigh__.add("conj", [j]); col_neigh__["conj"].append(i)
                            data_neigh__.add("conj", [neighbor__]); data_neigh__["conj"].append(neighbor__)
        ...
        row_neigh__.add("all", []); col_neigh__.add("all", []); data_neigh__.add("all", [])
        ...
        for i in row_neigh__.keys():
            template__.neighbor.add(i, sparse.csr_matrix((data_neigh__[i], (row_neigh__[i], col_neigh__[i])), \
                                (len(list(self.cells.keys())), len(list(self.cells.keys()))), dtype = int))
        for i in row_bound__.keys():
            template__.boundary.add(i, sparse.csr_matrix((data_bound__[i], (row_bound__[i], col_bound__[i])), \
                                (len(list(self.cells.keys())), len(list(self.cells.keys()))), dtype = int))
    ...
    def __getGeom(self):
        ...
        dCF_input__ = [[], [], []]
        eCF_input__ = [[], [], []]
        for i, j, k in itertools.izip(self.templates.neighbor["all"].row, \
                       self.templates.neighbor["all"].col, self.templates.neighbor["all"].data):
            ...
            Tf__ = Sf__ - Ef__
            # input
            Sf_input__[0].append([i, k, Sf__[0]]); Sf_input__[1].append([i, k, Sf__[1]]); Sf_input__[2].append([i, k, Sf__[2]])
            Ef_input__[0].append([i, k, Ef__[0]]); Ef_input__[1].append([i, k, Ef__[1]]); Ef_input__[2].append([i, k, Ef__[2]])
            Tf_input__[0].append([i, k, Tf__[0]]); Tf_input__[1].append([i, k, Tf__[1]]); Tf_input__[2].append([i, k, Tf__[2]])
            dCf_input__[0].append([i, k, dCf__[0]]); dCf_input__[1].append([i, k, dCf__[1]]); dCf_input__[2].append([i, k, dCf__[2]])
            eCf_input__[0].append([i, k, eCf__[0]]); eCf_input__[1].append([i, k, eCf__[1]]); eCf_input__[2].append([i, k, eCf__[2]])
            dCF_input__[0].append([i, j, dCF__[0]]); dCF_input__[1].append([i, j, dCF__[1]]); dCF_input__[2].append([i, j, dCF__[2]])
            eCF_input__[0].append([i, j, eCF__[0]]); eCF_input__[1].append([i, j, eCF__[1]]); eCF_input__[2].append([i, j, eCF__[2]])
        for i in [0, 1, 2]:
            Sf_input__[i] = list(np.transpose(np.array(Sf_input__[i])))
            Ef_input__[i] = list(np.trasnpose(np.array(Ef_input__[i])))
            Tf_input__[i] = list(np.trasnpose(np.array(Tf_input__[i])))
            dCf_input__[i] = list(np.trasnpose(np.array(dCf_input__[i])))
            eCf_input__[i] = list(np.trasnpose(np.array(eCf_input__[i])))
            dCF_input__[i] = list(np.trasnpose(np.array(dCF_input__[i])))
            eCF_input__[i] = list(np.trasnpose(np.array(eCF_input__[i])))
    ...
    def __getClust(self, *args):
        ...
        for i in s2s_dict_keys__:
            ...
            self.clusts.add(index, clust( dict({"q": np.array([0.00, 0.00])}), args[0][i]))
        ...
        return
    def __getInfo(self, *args):
        # args mesh : meshio.mesh, user : user
        # mesh.cells_dict based into face_id, cell_id based
        cell_sets_keys = list(args[0].cell_sets_dict.keys())
        ...
        for i in mesh.cells_dict.keys():
            ...
            if mesh.cells_dict[i][0].shape[0] < 5:
                for j in cell_sets_keys:
                    if i in list(args[0].cell_sets_dict[j].keys()):
                        if i not in list(face_bound_dict.keys()):
                            face_bound_dict.add(j, np.array(list(map(lambda x: x + face_id__, \
                                                args[0].cell_sets_dict[j][i]))))
                        else:
                ...
                for j in cell_sets_keys:
                    if i in list(args[0].cell_sets_dict[j].keys()):
                        if i not in list(cell_domain_dict.keys()):
                            cell_domain_dict.add(j, np.array(list(map(lambda x: x + cell_id__, \
                                                 args[0].cell_sets_dict[j][i]))))
                        else:
        ...
        for i in cell_domain_dict.keys():
            for j in self.cells.keys():
                if j in cell_domain_dict[i]:
                    self.cells[j].appendDomain(i, args[1])
        ...
        if "fluid" in list(self.template.neighbor.keys()):
            for i, j, k in itertools.izip(self.template.neighbor["fluid"].row, self.template.neighbor["fluid"].col, \
                           self.template.neighbor["fluid"].data):
                self.faces[k].appendBound("", args[1], "fluid")
        elif "solid" in list(self.template.neighbor.keys()):
            for i, j, k in itertools.izip(self.template.neighbor["solid"].row, self.template.neighbor["solid"].col, \
                           self.template.neighbor["solid"].data):
                self.faces[k].appendBound("", args[1], "solid", self.cells[i].domain)
        elif "conj" in list(self.template.neighbor.keys()):
            for i, j, k in itertools.izip(self.template.neighbor["conj"].row, self.template.neighbor["conj"].col, \
                           self.template.neighbor["conj"].data):
                ...
                for l in face_bound_dict.keys():
                    if k in face_bound_dict[l]:
                        self.faces[k].appendBound(l, args[1], "conj", self.cells[issolid].domain)
                        break
        if "fluid" in list(self.template.boundary.keys()):
            for i, j in itertools.izip(self.template.boundary["fluid"].row, self.template.neighbor["fluid"].col):
                for k in face_bound_dict.keys():
                    if j in face_bound_dict[k]:
                        self.faces[j].appendBound(k, args[1], "fluid")
                        break
        elif "solid" in list(self.template.boundary.keys()):
            for i, j in itertools.izip(self.template.boundary["solid"].row, self.template.neighbor["solid"].col):
                for k in face_bound_dict.keys():
                    if j in face_bound_dict[k]:
                        self.faces[j].appendBound(k, args[1], "solid", self.cells[i].domain)
                        break
...
def make_scheme(*args):
    # args init_file : str, solid_prop_file : str, const_value_file : str, mesh_file : str
    user__ = user(args[0], args[1], args[2])
    mesh__ = mesh(args[3], user__)
    return user__, mesh__

if __name__ == "__main__":
    filename = [x for x in input("").split(" ")]
