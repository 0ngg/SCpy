...
class solver:
    def __init__(self, *args):
        # 
        # args init_file : str, solid_prop_file : str, const_value_file : str, mesh_file : str, what : np.array([], dtype = str),
        ...
        for i in args[4]:
            if var_to_linear[i] == "momentum":
                self.__eq.add(i, cfd_linear.__dict__[var_to_linear[i]](self.__mesh, coor_dict[i]))
            else:
                self.__eq.add(i, cfd_linear.__dict__[var_to_linear[i]](self.__mesh))
            self.__res_loop.add(i, [])
            self.__res_time.add(i, [])
        self.__current_time = 0
        self.__under_relax = args[5]
        self.__tol = args[6]
        self.__max_iter = args[7]
        self.__time_step = args[8]
    def __exportsolver(self):
        
        
        return
    def __energys2sloop(self):
        ...
        while all(check) is False:
            ...
            rmsr_t__ = self.__eq["T"].__itersolve(self.__eq["T"], self.__under_relax, self.__tol, self.__max_iter, self.__time_step, self.__user, self.__current_time)
            rmsr_q__ = self.__eq["q"].__itersolve(self.__eq["q"], self.__under_relax, self.__tol, self.__max_iter, self.__time_step, self.__current_time)
            self.__res_loop["T"].extend(rmsr_t__); self.__res_loop["q"].extend(rmsr_q__)
            
            check = [i < self.__tol for i in [rmsr_t__, rmsr_q__]]
            passes += 1
        return passes
    def __SIMPLEloop(self):
        # stop condition -> all equations return rmsr value < tol
        passes = 0
        check = [False, False, False, False]
        while all(check) is False:
            # args mesh : mesh, under_relax : double, tol : double, max_iter : int, time_step : float, user : user
            rmsr_u__ = self.__eq["u"].__itersolve(self.__eq["u"], self.__under_relax, self.__tol, self.__max_iter, self.__time_step, self.__user, self.__current_time)
            rmsr_v__ = self.__eq["v"].__itersolve(self.__eq["u"], self.__under_relax, self.__tol, self.__max_iter, self.__time_step, self.__user, self.__current_time)
            rmsr_w__ = self.__eq["w"].__itersolve(self.__eq["u"], self.__under_relax, self.__tol, self.__max_iter, self.__time_step, self.__user, self.__current_time)
            # args mesh : mesh, under_relax : double, tol : double, max_iter : int, time_step : float, u :momentum, v : momentum, w : momentum
            rmsr_pcor__ = self.__eq["Pcor"].__itersolve(self.__eq["Pcor"], self.__under_relax, self.__tol, self.__max_iter, \
    ...
    def __turbloop(self):
        ...
        while all(check) is False:
            # args mesh : mesh, under_relax : double, tol : double, max_iter : int, time_step : float, user : user
            rmsr_k__ = self.__eq["k"].__itersolve(self.__eq["k"], self.__under_relax, self.__tol, self.__max_iter, self.__time_step, self.__user, self.__current_time)
            rmsr_e__ = self.__eq["e"].__itersolve(self.__eq["e"], self.__under_relax, self.__tol, self.__max_iter, self.__time_step, self.__user, self.__current_time)
            self.__res_loop["k"].extend(rmsr_k__); self.__res_loop["e"].extend(rmsr_e__)
            check = [i < self.__tol for i in [rmsr_k__, rmsr_e__]]
            passes += 1
        return passes
    def __sctimeloop(self, *args):
        ...
        while n_loops1 > args[0] * 3:
            n_loops1 = 0
            n_loops1 += self.__energys2sloop()
            while n_loops2 > args[0] * 2:
                n_loops2 = 0
                n_loops2 += self.__SIMPLEloop()
                n_loops2 += self.__turbloop()
            n_loops1 += n_loops2
    ...
    def __scsteadyloop(self, max_passes_multip : int, max_time_steps = 1000):
        ...
        while check_time is False:
            check_time = self.__sctimeloop(self, max_passes_multip)
            pass_time += 1
            if pass_time >= max_time_steps:
                break
        self.__exportsolver(self)
        return
