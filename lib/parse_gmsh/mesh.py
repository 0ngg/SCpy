...
class Mesh(object):
    ...
    def get_number_of_physical_groups(self):
        return len(list(self.physical_groups_.keys()))

    def add_physical_group(self, physical_group: PhysicalEntity):
        tag = physical_group.get_tag()
        self.physical_groups_[tag] = physical_group

    def get_physical_group(self, tag: int):
        return self.physical_groups_[tag]
    ...
    def has_element_entity(self, dim: int, tag: int, type: int) -> bool:
        """Test does mesh have element entity with `(dim, tag)`."""
        return (dim, tag, type) in self.element_entities_
    ...
    def add_element_entity(self, element_entity: ElementEntity):
        ...
        type = element_entity.get_element_type()
        self.element_entities_[(dim, tag, type)] = element_entity

    def get_element_entity(self, dim: int, tag: int, type: int) -> ElementEntity:
        """Get element entity based on dimension `dim` and tag `tag`."""
        return self.element_entities_[(dim, tag, type)]
    ...
    def __str__(self):
        ...
        io.write("Physical groups: %s\n" % self.get_number_of_physical_groups())
