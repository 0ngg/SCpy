...
class cell:
    ...
    def __getcentroid():
        ...
        return
    def __getvolume(self, spec, *args):
...
class mesh:
    ...
    def __call__(self, *args):
        return self.__dict__[args[0]]
    def __getnodes(self, spec):
        return
    def __getcells(self, spec):
        return
    def __getfaces(self, spec):
        return
    def __getclusts(self, spec):
        return
    def __getgeoms(self, spec):
        return
    def __gettemplates(self, spec):
        return
...
class user:
    ...
    def __call__(self, *args):
        # args var name : str, col name : str, row : int
        return self.__dict__[args[0]][args[1]][args[2]] 
...
class linear:
    ...
    def _singlesolve(self, *args):
        ...
        for i in range(0, x.shape(0)):
            args[0]("cells")[i]("value")[args[1]][-1] = x[i][0]
        return
...
class momentum(linear):
    ...
    def __calcbound(self, *args):
        ...
        if "noslip" in args[1]:
            ...
            grad_vin_v1_ = np.dot(args[0]("cells")[args[2]]("grad")[axes[1]][-1] - \
            ...
            grad_vout_v1_ = np.dot(args[0]("cells")[args[2]]("grad")[axes[0]][-1] - \
                          (np.dot(args[0]("cells")[args[2]]("grad")[axes[0]][-1], eCf__) * eCf__))
            grad_vout_v1_ = np.dot(args[0]("cells")[args[2]]("grad")[axes[1]][-1] - \
                          (np.dot(args[0]("cells")[args[2]]("grad")[axes[1]][-1], eCf__) * eCf__))
...
class turb_k(linear):
    ...
    def __calcbound(self, *args):
        ...
        if "inlet" in args[1]:
            ...
            grad_vin_v1_ = np.dot(args[0]("cells")[args[2]]("grad")["w"][-1] - \
            ...
            grad_vout_v1_ = np.dot(args[0]("cells")[args[2]]("grad")["v"][-1] - \
                          (np.dot(args[0]("cells")[args[2]]("grad")["v"][-1], eCf__) * eCf__))
            grad_vout_v1_ = np.dot(args[0]("cells")[args[2]]("grad")["w"][-1] - \
                          (np.dot(args[0]("cells")[args[2]]("grad")["w"][-1], eCf__) * eCf__))
